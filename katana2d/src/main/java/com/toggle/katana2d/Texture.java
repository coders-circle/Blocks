...
public class Texture {
    ...
    public void draw(int textureId, GLRenderer renderer, float x, float y, float z, float angle,
                     float scaleX, float scaleY, float clipX, float clipY,
        ...
        GLES20.glVertexAttribPointer(
                renderer.mSpritePositionHandle, 2, GLES20.GL_FLOAT, false, 2 * 4, 0);

        GLES20.glUniform4f(renderer.mSpriteClipHandle, clipX, clipY, clipW, clipH);
        renderer.setSpriteTransform(
                x, y, z, width * scaleX, height * scaleY, angle, originX, originY);
