    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                ...
                pointer.x = (e.getX(pointerIndex) - mRenderer.getViewportX()) /
                        mRenderer.getViewportScale();
                pointer.y = (e.getY(pointerIndex) - mRenderer.getViewportY()) /
                        mRenderer.getViewportScale();
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int size = e.getPointerCount(), i = 0; i < size; i++) {
                    TouchInputData.Pointer pointer =
                            mTouchInputData.pointers.get(e.getPointerId(i));
                    if (pointer != null) {
                        float lastX = pointer.x;
                        float lastY = pointer.y;
                        pointer.x = (e.getX(i) - mRenderer.getViewportX()) /
                                mRenderer.getViewportScale();
                        pointer.y = (e.getY(i) - mRenderer.getViewportY()) /
                                mRenderer.getViewportScale();
