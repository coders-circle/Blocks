...
public class GLRenderer implements GLSurfaceView.Renderer {
    ...
    private void prepareSpriteProgram() {
        ...
        int vertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
                Utilities.getRawFileText(mContext, R.raw.vs_sprite));
        int fragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
                Utilities.getRawFileText(mContext, R.raw.fs_sprite));
        ...
        mSpriteVertexBuffer = ByteBuffer.allocateDirect(mSquareCoords.length*4).order(
                ByteOrder.nativeOrder()).asFloatBuffer();
        mSpriteVertexBuffer.put(mSquareCoords).position(0);

        // Create the index buffer
        mSpriteIndexBuffer = ByteBuffer.allocateDirect(mSquareDrawOrder.length*2).order(
        ...
        GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, mSpriteVertexBuffer.capacity() * 4,
                mSpriteVertexBuffer, GLES20.GL_STATIC_DRAW);
        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0);

        GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, mSpriteBO[1]);
        GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER, mSpriteIndexBuffer.capacity() * 2,
                mSpriteIndexBuffer, GLES20.GL_STATIC_DRAW);
    ...
    private void preparePointsSpriteProgram() {
        ...
        int pVertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
                Utilities.getRawFileText(mContext, R.raw.vs_point_sprite));
        int pFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
                Utilities.getRawFileText(mContext, R.raw.fs_point_sprite));
    ...
    private void preparePostProcessProgram() {
        // Compile the sprite shaders
        int vertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
                Utilities.getRawFileText(mContext, R.raw.vs_post_process));
        int fragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
                Utilities.getRawFileText(mContext, R.raw.fs_post_process));
        ...
        mPostProcessMVPMatrixHandle =
                GLES20.glGetUniformLocation(mPostProcessProgram, "uMVPMatrix");
    ...
    private void createFBO() {
        ...
        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, devWidth, devHeight,
        ...
        GLES20.glRenderbufferStorage(
                GLES20.GL_RENDERBUFFER, GLES20.GL_DEPTH_COMPONENT16, devWidth, devHeight);
        ...
        GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0,
                GLES20.GL_TEXTURE_2D, mRboTexture, 0);
        GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER,
                GLES20.GL_DEPTH_ATTACHMENT, GLES20.GL_RENDERBUFFER, mRboDepth);
    ...
    public void onDrawFrame(GL10 unused) {
        ...
        if (enablePostProcessing) {
            ...
            GLES20.glVertexAttribPointer(
                    mPostProcessPositionHandle, 2, GLES20.GL_FLOAT, false, 2 * 4, 0);
    ...
    public void setSpriteTransform(float posX, float posY, float z, float scaleX, float scaleY,
                                   float angle, float originX, float originY) {
        // calculate transformation matrix that will transform the vertices of the square

        // mModelMatrix =
        //      Translate(posX, posY) * Rotate(angle) *
        //      Translate(-originX, -originY) * Scale(scaleX, scaleY)
    ...
    public MultiTexture addTexture(int resourceId, float width, float height,
                                   int numCols, int numRows) {
        MultiTexture t = new MultiTexture(
                loadTexture(resourceId, (int)width, (int)height, numCols, numRows),
    ...
    private void reloadTexture(Texture t) {
        if (t.getClass() == SimpleBitmapTexture.class) {
            ...
            tt.textureIds = loadTexture(
                    tt.resourceId, (int)tt.width, (int)tt.height, tt.numCols, tt.numRows);
        }
    }
